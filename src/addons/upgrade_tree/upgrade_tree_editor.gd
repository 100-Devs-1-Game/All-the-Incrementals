...
var upgrade_treee_editor_instance
var current_scene: Node = null
var current_selected_node: GraphNode = null


func _enter_tree() -> void:
	upgrade_treee_editor_instance = UpgradeTreeEditor.instantiate()
	# Add the main panel to the editor's main viewport.
	EditorInterface.get_editor_main_screen().add_child(upgrade_treee_editor_instance)

	# Hide the main panel. Very much required.
	_make_visible(false)
	current_scene = EditorInterface.get_edited_scene_root()
	scene_changed.connect(_on_scene_changed)
	upgrade_treee_editor_instance.get_node("AddUpgrade").pressed.connect(_on_add_upgrade_pressed)
	upgrade_treee_editor_instance.get_node("GraphEdit").connection_request.connect(
...
func _on_add_upgrade_pressed() -> void:
	...
	if current_scene is UpgradeTree:
		...
		upgrade_treee_editor_instance.get_node("GraphEdit").add_child(graph_node)
	else:
...
func _on_connection_request(from_node, from_port, to_node, to_port):
	...
	for con in upgrade_treee_editor_instance.get_node("GraphEdit").get_connection_list():
		if con.to_node == to_node and con.to_port == to_port:
			return
	upgrade_treee_editor_instance.get_node("GraphEdit").connect_node(
		from_node, from_port, to_node, to_port
	)


func _exit_tree() -> void:
	if upgrade_treee_editor_instance:
		upgrade_treee_editor_instance.queue_free()
...
func _make_visible(visible: bool) -> void:
	if upgrade_treee_editor_instance:
		if visible:
			upgrade_treee_editor_instance.show()
		else:
			upgrade_treee_editor_instance.hide()
