...
func _process(delta: float) -> void:
	if inst.build_mode:
		...
		if follow:
			global_position = global_position.move_toward(follow.global_position, transition_speed * delta)

	if not inst.build_mode and follow:
		if Input.is_action_pressed("left"):
			follow.progress_ratio = clamp(follow.progress_ratio - (direction_speed * 0.001) * delta, 0.0, 1.0)
