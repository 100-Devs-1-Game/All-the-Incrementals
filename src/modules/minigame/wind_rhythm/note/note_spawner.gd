...
func spawn_note(position: float, rotation: float, lane_position: int):
	var judgment_x = get_node(judgment_line).global_position.x
	var note_instance = NOTE.instantiate()
	note_instance.global_position = Vector2(judgment_x + position, lane_position)
	note_instance.speed = judgment_x / conductor.seconds_per_beat * scroll_speed

	note_instance.rotate(rotation)
